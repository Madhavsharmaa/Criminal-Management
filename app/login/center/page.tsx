...
export default function CenterLoginPage() {
  ...
  useEffect(() => {
    const s = getSession();
    if (s && s.role === "Center") router.replace("/center/dashboard");
  }, [router]);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const data = await api.post<{
        ...
        name: string;
        email: string;
      }>("/auth/center/login", { email, password });
      setSession(data);
    ...
    <main className="paper-texture flex min-h-screen items-center justify-center bg-paper px-6">
      <div className="w-full max-w-sm">
        <div className="mb-8 text-center">
          <span className="stamp-badge mx-auto mb-4 inline-flex border-ink text-ink text-xs">Field Office</span>
          <h1 className="font-display text-2xl font-bold text-ink">Center Login</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 rounded-xl border border-ink/10 bg-paper-card p-7 shadow-card">
          ...
          <Input
            ...
            placeholder="center@department.gov"
          />
          <Input
            id="password"
            label="Password"
        ...
        <p className="mt-6 text-center text-sm text-ink/50">
          Admin instead?{" "}
          <Link href="/login/admin" className="font-semibold text-stamp hover:underline">
            Admin login
          </Link>
        </p>
